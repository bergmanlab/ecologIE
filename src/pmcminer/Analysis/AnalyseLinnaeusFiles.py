...
def write_spp_numbers(spp_dict,  pure_path, doi_list, outFile = False):
    if outFile:
        f = open(outFile, "w")
        f.write("subject\tarticle\tdoi\tin_article\n")
    for subject in doi_list:
        ...
        for text in ids:
            if text["filename"] in spp_dict[subject]:
                num_spp = spp_dict[subject][text["filename"]][-1]
            else:
                num_spp = 0
            f.write("\t".join([subject, text["filename"], text["doi"], str(num_spp),"\n"]))
    f.close()
...
def get_linnaeus_stats(data_dir, pure_dir, tags_dir, analysis_dir,
    ...
    write_spp_numbers(L.species, os.path.join(data_dir, pure_dir), doi_list, os.path.join(data_dir, analysis_dir, spp_per_article))
    write_spp_numbers(L.interactions,  os.path.join(data_dir, pure_dir), doi_list, os.path.join(data_dir, analysis_dir, interactions_per_article))
